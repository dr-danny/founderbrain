...
test("proxies only a fixed API origin, forwards the Hexclave token and allowlisted headers, drops the rest, no-store", async () => {
  ...
  const worker = createFounderBrainWorker(async (input, init) => {
    seen = { url: String(input), headers: new Headers(init?.headers), method: init?.method };
    return new Response(JSON.stringify({ ok: true }), {
      headers: { "Cache-Control": "public, max-age=999", "X-From-Upstream": "yes" },
    });
  });
  const request = new Request("https://app.example.test/api/brain?version=4", {
    method: "PUT",
    headers: {
      ...TOKEN,
      Authorization: "Bearer stale-token",
      "Content-Type": "application/json",
      "X-FounderBrain-Origin": "attacker",
      "X-Forwarded-Host": "attacker.test",
      "X-Request-Id": "client-spoofed",
      Cookie: "stack-refresh-token=browser-cookie",
    },
    body: "{}",
  });
  const result = await worker.fetch(request, baseEnv());
  assert.equal(seen.url, "https://founderbrain-api.up.railway.app/api/brain?version=4");
  ...
  assert.equal(
    ...
    null,
    "nothing reads Authorization, so it does not cross",
  );
  assert.equal(
    seen.headers?.get("cookie"),
    null,
    "the Hexclave refresh cookie stays in the browser; the access token header is what the API verifies",
  );
  assert.equal(seen.headers?.get("x-forwarded-host"), null);
  assert.notEqual(
    seen.headers?.get("x-request-id"),
    "client-spoofed",
    "edge mints X-Request-Id; never forwards the client value",
  );
  assert.match(seen.headers?.get("x-request-id") ?? "", /^[0-9a-f-]{36}$/i);
  assert.equal(
    result.headers.get("x-request-id"),
    seen.headers?.get("x-request-id"),
    "response echoes the minted id for support",
  );
  assert.equal(result.headers.get("cache-control"), "private, no-store");
  assert.match(
    result.headers.get("content-security-policy") ?? "",
    /connect-src 'self' https:\/\/api\.hexclave\.com$/,
  );
});
test("mints a fresh X-Request-Id when the client omits one", async () => {
  ...
  const result = await worker.fetch(
    new Request("https://app.example.test/api/brain", { headers: TOKEN }),
    baseEnv(),
  );
...
test("rate limits bursty /api traffic per connecting IP", async () => {
  ...
  for (let i = 0; i < 200; i += 1) {
    const result = await worker.fetch(
      new Request("https://app.example.test/api/config", {
...
test("refuses /api without the token header before touching the origin, except /api/config which the browser needs first", async () => {
  ...
  const worker = createFounderBrainWorker(async () => {
    called += 1;
    return new Response(JSON.stringify({ authMode: "hexclave" }), {
      headers: { "Content-Type": "application/json" },
    });
  });
  const refused = await worker.fetch(new Request("https://app.example.test/api/brain"), baseEnv());
  assert.equal(refused.status, 401);
  assert.equal(((await refused.json()) as { error: string }).error, "sign_in_required");
  ...
  const configLike = await worker.fetch(
    new Request("https://app.example.test/api/config/../brain"),
    baseEnv(),
  );
  assert.equal(
    configLike.status,
    401,
    "URL normalisation happens before the check, so this is /api/brain",
  );
});
test("CSP only widens connect-src for a valid HTTPS Hexclave origin; otherwise it stays 'self'", async () => {
  ...
  const ok = await worker.fetch(
    new Request("https://app.example.test/", { headers: { Accept: "text/html" } }),
    { ...baseEnv(), HEXCLAVE_API_URL: "https://api.hexclave.com/" },
  );
  assert.match(
...
test("never follows an API redirect", async () => {
  const worker = createFounderBrainWorker(
  ...
  assert.equal(((await result.json()) as { error: string }).error, "upstream_redirect");
});
test("returns a bounded timeout when the fixed origin does not answer", async () => {
  ...
  assert.equal(((await result.json()) as { error: string }).error, "gateway_timeout");
});
...
test("serves SPA fallback through assets and does not proxy non-api paths", async () => {
  ...
  const result = await worker.fetch(
    new Request("https://app.example.test/mission/customer", { headers: { Accept: "text/html" } }),
    {
      ...baseEnv(),
      ASSETS: {
        async fetch(request) {
