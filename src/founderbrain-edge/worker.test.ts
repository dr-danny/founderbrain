...
const baseEnv = (): FounderBrainEdgeEnv => ({ API_ORIGIN: "https://founderbrain-api.up.railway.app", ORIGIN_SECRET: "test-origin-secret", AUTH_ORIGIN: "https://auth.example.test", ASSETS: { async fetch() { return new Response("asset"); } } });

test("proxies only a fixed API origin with allowlisted headers and no-store", async () => {
  ...
  const request = new Request("https://app.example.test/api/brain?version=4", { method: "PUT", headers: { Authorization: "Bearer user-token", "Content-Type": "application/json", "X-FounderBrain-Origin": "attacker", "X-Forwarded-Host": "attacker.test", "X-Request-Id": "req-7" }, body: "{}" });
  const result = await worker.fetch(request, baseEnv());
  assert.equal(seen.url, "https://founderbrain-api.up.railway.app/api/brain?version=4"); assert.equal(seen.method, "PUT"); assert.equal(seen.headers?.get("authorization"), "Bearer user-token"); assert.equal(seen.headers?.get("x-founderbrain-origin"), "test-origin-secret"); assert.equal(seen.headers?.get("x-forwarded-host"), null);
  assert.equal(result.headers.get("cache-control"), "private, no-store"); assert.match(result.headers.get("content-security-policy") ?? "", /https:\/\/auth\.example\.test/);
});
...
test("fails closed when origin configuration is missing, unsafe, or not a bare origin", async () => {
  ...
  const missing = await worker.fetch(new Request("https://app.example.test/api/brain"), { ASSETS: baseEnv().ASSETS });
  ...
  for (const origin of ["http://evil.example", "https://user:pass@api.example", "https://api.example/prefix", "https://api.example/?query=1", "https://api.example/#hash"]) {
    const rejected = await worker.fetch(new Request("https://app.example.test/api/brain"), { ...baseEnv(), API_ORIGIN: origin });
    assert.equal(rejected.status, 503, origin);
  }
});
test("permits local HTTP only through an explicit injected test option", async () => {
  const request = new Request("https://app.example.test/api/brain");
