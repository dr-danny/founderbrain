...
const request = (value: string | undefined, ip = '203.0.113.9') =>
  ({ headers: value === undefined ? {} : { [ACCESS_TOKEN_HEADER]: value }, ip }) as unknown as FastifyRequest;

describe('Hexclave authentication', () => {
  const config = { HEXCLAVE_PROJECT_ID: PROJECT, HEXCLAVE_API_URL: '', FOUNDERBRAIN_LOCAL_DEMO: 'false', NODE_ENV: 'test' } as Config;
  ...
  it('accepts a signed regular token and returns a project-scoped opaque subject plus the email', async () => {
    ...
    assert.doesNotMatch(identity.subject, /127\.0\.0\.1/, 'the API hostname must not be part of the workspace key');
  });
  ...
  it('refuses garbage, a wrong issuer, a wrong audience and an expired token', async () => {
    ...
    await assert.rejects(auth()(request(await token({ issuer: hexclaveEndpoints(apiUrl, OTHER_PROJECT).issuer }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ issuer: 'https://api.hexclave.com/api/v1/projects/' + PROJECT }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ audience: OTHER_PROJECT }))), { status: 401 });
  ...
  it('refuses anonymous and restricted sessions by issuer, by audience and by claim', async () => {
    ...
    await assert.rejects(auth()(request(await token({ issuer: `${origin}/api/v1/projects-anonymous-users/${PROJECT}`, audience: `${PROJECT}:anon`, claims: { is_anonymous: true, is_restricted: true } }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ issuer: `${origin}/api/v1/projects-restricted-users/${PROJECT}`, audience: `${PROJECT}:restricted`, claims: { is_restricted: true, restricted_reason: { type: 'email_not_verified' } } }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ claims: { is_anonymous: true } }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ claims: { is_restricted: true } }))), { status: 401 });
  });
  ...
  it('refuses a missing, malformed or unverified email', async () => {
    await assert.rejects(auth()(request(await token({ claims: { email: null } }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ claims: { email: 'not-an-email' } }))), { status: 401 });
    await assert.rejects(auth()(request(await token({ claims: { email_verified: false } }))), { status: 401 });
  });
  ...
  it('refuses to start without a project id outside the local demo', () => {
    assert.throws(() => createAuthenticator({ ...config, HEXCLAVE_API_URL: apiUrl, HEXCLAVE_PROJECT_ID: undefined }));
  });
...
describe('Hexclave configuration helpers', () => {
  it('derives the documented issuer and JWKS URL from the API origin and project id', () => {
    ...
    assert.equal(jwks.href, `https://api.hexclave.com/api/v1/projects/${PROJECT}/.well-known/jwks.json`);
    assert.equal(hexclaveEndpoints('https://api.hexclave.com/', PROJECT).issuer, issuer, 'a trailing slash changes nothing');
  });
