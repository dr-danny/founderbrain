...
export async function runFounderBrainMigrations(databaseUrl: string): Promise<void> {
  ...
  const sql = postgres(databaseUrl, {
    max: 1,
    onnotice: () => undefined,
    connection: { application_name: "founderbrain-migrate" },
  });
  try {
    await sql.begin(async (tx) => {
      ...
      const done = await tx<
        { name: string }[]
      >`select name from founderbrain.migrations where name = ${migrationName}`;
      if (done.length === 0) {
      ...
      if (rls.length !== 3 || rls.some((row) => !row.enabled || !row.forced)) {
        throw new Error(
          "FounderBrain row-level security is not enabled and forced on every FounderBrain table",
        );
      }
    });
  } finally {
...
/** Production runtime must not be superuser or bypass RLS. Callers pass production from Config. */
export async function assertSafeRole(sql: Sql, production: boolean): Promise<void> {
