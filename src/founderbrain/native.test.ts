...
const skip = !url
  ? "Native test database not configured"
  : process.env.FB_TEST_EMBEDDED === "true"
    ? "Requires native independent PostgreSQL connections; not claimed from PGlite"
    : undefined;
it("native concurrent delete/save leaves no content after deletion", { skip }, async () => {
  ...
  const store = new PgBrainStore(url!);
  try {
    for (let i = 0; i < 10; i++) {
      const subject = "native-race|" + randomUUID();
      const id = await store.ensureWorkspace(subject);
      const b = emptyBrain();
      b.identity.name = "Private fixture";
      const outcomes = await Promise.allSettled([
        store.commit(id, b, 0, "race-" + randomUUID()),
        store.deleteWorkspace(subject, id),
      ]);
