...
export async function buildApi(config:Config,options:{store?:PgBrainStore;jobs?:BrainJobs;authenticate?:Authenticate;serveWeb?:boolean;logger?:boolean}={}){
  ...
  const log = createFounderBrainLogger('api', enableLogger ? (process.env.LOG_LEVEL ?? 'info') : 'silent');
  const jobs=options.jobs??new BrainJobs(store,config,undefined,(event)=>logJobEvent(log,event));
  ...
  const app=Fastify({
    ...
    genReqId: (req) => {
      ...
      return typeof inbound === 'string' && inbound.length > 0 && inbound.length < 120 ? inbound : `api-${Date.now().toString(16)}`;
    },
  });
  ...
  app.addHook('onRequest',async(req,reply)=>{
    ...
    if (req.headers['x-request-id']) reply.header('X-Request-Id', String(req.headers['x-request-id']));
  ...
  app.get('/health/metrics',async()=>({
    ...
    rateLimit: { mode: 'in-memory-single-instance', ip: API_IP_LIMIT, subjectMutations: API_SUBJECT_MUTATION_LIMIT },
  }));
  ...
  app.setErrorHandler((error,req,reply)=>{
    if(error instanceof DomainError){
      if (error.status >= 500) log.warn({ reqId: req.id, code: error.code, status: error.status }, 'founderbrain.domain_error');
      return reply.code(error.status).send({error:error.code,message:error.message,...(error.details?{details:error.details}:{})});
    }
