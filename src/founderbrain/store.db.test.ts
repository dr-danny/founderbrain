...
const skip = databaseUrl ? undefined : 'FB_TEST_DATABASE_URL is not set, so real Postgres storage is not being claimed';
...
function filledBrain() {
  ...
  brain.identity = { name: 'Ada', venture: 'Northwind', role: 'Founder', stage: 'building', goal: 'Validate demand', approved: true };
  return brain;
}

describe(process.env.FB_TEST_EMBEDDED === 'true' ? 'PgBrainStore on embedded PostgreSQL (not native concurrency proof)' : 'PgBrainStore against native Postgres', () => {
  it('creates an empty isolated workspace and commits encrypted revision history', { skip }, async () => {
    assert.ok(store);
    ...
    assert.equal(replay.version, 1, 'idempotent replay returns the original revision, not latest');
  ...
  it('rejects stale writes and idempotency reuse with another body', { skip }, async () => {
    ...
    await assert.rejects(store.commit(workspace, changed, 0, 'stale-save'), { code: 'version_conflict' });
    await assert.rejects(store.commit(workspace, changed, 1, 'save-1'), { code: 'idempotency_mismatch' });
  });
  ...
  it('keeps ge rows tenant-scoped under RLS', { skip }, async () => {
    ...
    try {
      ...
      const visible = await store.scoped(workspace, (tx) => tx<{ founder_id: string }[]>`select founder_id from ge_file`);
      assert.deepEqual(visible.map((row) => row.founder_id), [workspace]);
    } finally {
