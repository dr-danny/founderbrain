...
import { createFounderKey, openBlob, sealBlob, sha256Hex, unwrapDataKey } from '../server/storage/crypto.ts';
import {
  type Artifact,
  type Brain,
  type BrainState,
...
type FounderRow = { version: number | string | bigint; wrapped_key: Uint8Array; deleted_at: Date | string | null };
type FileRow = {
  blob_sha: string;
...
function asVersion(value: number | string | bigint): number {
  ...
  if (!Number.isSafeInteger(n) || n < 0) throw fail(503, 'storage_unavailable', 'Saved workspace data is invalid. Please try again.');
  return n;
}
...
export class PgBrainStore {
  ...
  private async ready(): Promise<void> {
    try {
      this.roleCheck ??= assertSafeRole(this.sql);
  ...
  async ensureWorkspace(subject: string): Promise<string> {
    ...
    try {
      return await this.sql.begin(async (tx) => {
        ...
        `;
        if (existing[0]) {
          if (existing[0].deleted_at !== null) throw fail(410, 'workspace_deleted', 'This workspace was deleted.');
          const membership = await tx`select 1 from fb_member where subject=${subject} and founder_id=${existing[0].founder_id} and revoked_at is null`;
          if (!membership.length) throw fail(403, 'membership_revoked', 'Workspace access was revoked.');
        ...
        if (inserted.length === 0) {
          ...
          `;
          if (!winner[0] || winner[0].deleted_at !== null) throw fail(410, 'workspace_deleted', 'This workspace was deleted.');
  ...
  async read(workspaceId: string, version?: number): Promise<BrainState> {
    await this.ready();
    validOpaque(workspaceId, 'workspace id', 64);
    if (version !== undefined && (!Number.isSafeInteger(version) || version < 0)) throw fail(422, 'invalid_version', 'Version is invalid.');
    try {
  ...
  private async readInternal(workspaceId: string, requestedVersion?: number): Promise<BrainState> {
    return await this.sql.begin(async (tx) => {
      ...
      if (founder.deleted_at !== null) throw fail(410, 'workspace_deleted', 'This workspace was deleted.');
      const currentVersion = asVersion(founder.version);
      const target = requestedVersion ?? currentVersion;
      if (target === 0) {
        if (requestedVersion !== undefined && currentVersion > 0) throw fail(404, 'version_not_found', 'Version was not found.');
      ...
      await tx`select set_config('app.founder_id', ${workspaceId}, true)`;
      const files = requestedVersion === undefined
        ? await tx<FileRow[]>`
        ...
        : await tx<FileRow[]>`
            select v.blob_sha, b.ciphertext, b.nonce, v.version, v.at
             ...
             where v.founder_id = ${workspaceId} and v.path = ${BRAIN_PATH} and v.version = ${target} and v.deleted = false
             limit 1
          `;
      const file = files[0];
  ...
  async commit(workspaceId: string, value: Brain, expectedVersion: number, idempotencyKey: string): Promise<BrainState> {
    const brain = checkedBrain(value);
    return await this.commitWithReceipt(workspaceId, brain, expectedVersion, idempotencyKey, requestHash(brain, expectedVersion));
  }
  ...
  private async commitWithReceipt(
    ...
    if (!Number.isSafeInteger(expectedVersion) || expectedVersion < 0) throw fail(422, 'invalid_version', 'Version is invalid.');
    ...
    try {
      committedVersion = await this.sql.begin(async (tx) => {
        ...
        if (!founder) throw fail(404, 'workspace_not_found', 'Workspace was not found.');
        if (founder.deleted_at !== null) throw fail(410, 'workspace_deleted', 'This workspace was deleted.');
        const receipts = await tx<{ request_hash: string; result_version: number | string | bigint }[]>`
        ...
        `;
        if (receipts[0]) {
          if (receipts[0].request_hash !== bodyHash) throw fail(409, 'idempotency_mismatch', 'This idempotency key was already used for a different save.');
          return asVersion(receipts[0].result_version);
        }
        const current = asVersion(founder.version);
        if (current !== expectedVersion) throw fail(409, 'version_conflict', 'This workspace changed. Reload and compare before saving.');
        ...
        await tx`
          ...
          values (${workspaceId}, ${BRAIN_PATH}, ${sealed.sha}, ${sealed.sizeBytes}, now(), ${next})
          on conflict (founder_id, path) do update set
            blob_sha = excluded.blob_sha, size_bytes = excluded.size_bytes, mtime = excluded.mtime, version = excluded.version
        `;
        await tx`
          insert into ge_file_version (founder_id, path, version, blob_sha, size_bytes, verb, deleted)
          values (${workspaceId}, ${BRAIN_PATH}, ${next}, ${sealed.sha}, ${sealed.sizeBytes}, 'founderbrain.save', false)
        `;
        const updated = await tx<{ version: number | string | bigint }[]>`
          update founder set version = ${next} where id = ${workspaceId} and version = ${current} returning version
        `;
        if (!updated[0]) throw fail(409, 'version_conflict', 'This workspace changed. Reload and compare before saving.');
        await tx`
    ...
    try {
      ...
      throw new DomainError(503, 'verification_pending', `Save committed as version ${String(committedVersion)} but verification is pending.`, {committedVersion});
    }
  }
  ...
  async history(workspaceId: string): Promise<Array<{ version: number; sha: string; at: string }>> {
    ...
    try {
      return await this.sql.begin(async (tx) => {
        const founders = await tx<FounderRow[]>`select wrapped_key, version, deleted_at from founder where id = ${workspaceId} limit 1`;
        const founder = founders[0];
        if (!founder) throw fail(404, 'workspace_not_found', 'Workspace was not found.');
        if (founder.deleted_at !== null) throw fail(410, 'workspace_deleted', 'This workspace was deleted.');
        await tx`select set_config('app.founder_id', ${workspaceId}, true)`;
        ...
        const key = unwrapDataKey(workspaceId, founder.wrapped_key);
        return rows.map((row) => {
          const brain = checkedBrain(JSON.parse(openBlob(workspaceId, key, row.blob_sha, row.ciphertext, row.nonce).toString('utf8')) as Brain);
          const at = iso(row.at);
  ...
  async restore(workspaceId: string, version: number, expectedVersion: number, idempotencyKey: string): Promise<BrainState> {
    if (!Number.isSafeInteger(version) || version <= 0) throw fail(422, 'invalid_version', 'Version is invalid.');
    ...
    return await this.commitWithReceipt(
      ...
      expectedVersion,
      idempotencyKey,
      sha256Hex(Buffer.from(canonicalize({ operation: 'restore', version, brain: prior.brain, expectedVersion }), 'utf8')),
    );
  }
  ...
  async deleteWorkspace(subject: string, workspaceId: string): Promise<void> {
    ...
    try {
      await this.sql.begin(async (tx) => {
        ...
        if (!binding || binding.founder_id !== workspaceId) throw fail(404, 'workspace_not_found', 'Workspace was not found.');
        ...
        if (jobSchema[0]?.present) {
          const pending = await tx`select id,status,reserved,budget_day,provider_request_id from fb_ai_job where founder_id=${workspaceId} and status in ('queued','running','uncertain') for update`;
          for (const job of pending) {
            if (job.status === 'queued') {
              for (const scope of ['global','workspace:'+workspaceId]) await tx`update fb_budget set reserved=greatest(0,reserved-${job.reserved}) where scope=${scope} and day=${job.budget_day}`;
            } else {
              await tx`insert into fb_usage_reconciliation(job_id,scope,budget_day,reserved,provider_request_id,reason) values(${job.id},${'workspace:'+workspaceId},${job.budget_day},${job.reserved},${job.provider_request_id},'workspace_deleted_during_uncertain_call') on conflict do nothing`;
            }
          }
